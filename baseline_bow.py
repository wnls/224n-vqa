...
def train(model, optim, loader):
  ...
  for i,batch in enumerate(loader):
    ...
    if loss_type == 'BCE':
      ...
      out = out.view(q_embed.size(0), q_embed.size(1))
      diff = torch.clamp(out[:,1]-out[:,0]+0.6, min=0) + torch.clamp(out[:,2]-out[:,0]+0.6, min=0) + torch.clamp(out[:,3]-out[:,0]+0.6, min=0)
      loss = diff.mean()
    # update stats
...
if __name__ == '__main__':
  ...
  if not os.path.exists(out_dir):
    os.mkdir(out_dir)
  file_format = os.path.join(out_dir, 'lr{:f}_wd{:f}_bts{:d}_{:s}'.format(lr, wd, batch_size, loss_type))
  ...
  if loss_type == 'BCE':
    ...
    raise NotImplementedError("Please use BCE or rank loss." )
